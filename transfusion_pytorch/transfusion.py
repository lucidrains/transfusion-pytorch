...
def transfusion_attn_mask(modalities: Int['b m 3']):
    modalities = modalities.long().tolist()

    def mask_mod(b, h, q_idx, kv_idx):
        is_causal = causal(b, h, q_idx, kv_idx)

        modality_mask_mods = [modality(offset, length) for _, offset, length in modalities[b]]

        is_modality = any([fn(b, h, q_idx, kv_idx) for fn in modality_mask_mods])
...
class Attention(Module):
    ...
    def forward(
        ...
        assert not (exists(flex_attn_score_mod) and exists(attn_mask))
        ...
        if self.use_flex_attn:
            ...
            flex_attn_kwargs = dict()

            if self.softcap_value > 0.:
                flex_attn_kwargs(score_mod = softcap_score_mod(self.softcap_value))

            out = flex_attention(q, k, v, block_mask = block_mask, score_mod = softcap_score_mod)
    ...
    def forward(
        ...
        if exists(modality_positions):
            if self.use_flex_attn:
                ...
                block_mask = create_block_mask(transfusion_mask_fn, B = None, H = None, Q_LEN = seq_len, KV_LEN = seq_len)
